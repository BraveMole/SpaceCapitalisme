...
public class Vaisseau extends SuperActor implements HasInventory {
	...
	private void AllTradesToCargo(boolean charger) {
		...
		for (int i = 0;i<size;i++) {
			if (charger) {
				if(!this.TradeToCargo(this.listeTrade.get(i),charger)) {
					System.out.println("Erreur AllTradesToCargo");
				}
			}
			else {
				if(!this.TradeToCargo(this.listeTrade.pop(),charger)) {
					System.out.println("Erreur AllTradesToCargo");
				}
			}
	...
	public void draw(Batch batch, float parentAlpha)
	{
		...
		else if(worldZoom>palierZoom[1]*200){
			float scale = palierZoom[1]*200 + (float) Math.sqrt(worldZoom-palierZoom[1]*200);
