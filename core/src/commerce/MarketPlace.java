...
import static gameConcepts.Ressource.*;

public class MarketPlace {
	static private float cycleTime=20f;
	static private float timeElapsed=21f;
	static private long timer;
	private static float cycleRechercheVaisseau=1f;
	...
	static private Array<Offre> concatenateSortedOffreList1;
    static private Array<Offre> concatenateSortedOffreList2;
	static private MapIndex currentIndex;
	static public int nbTrade;
    private static int nbTrader=0;
	...
	static public int getXindex(float x){
		for (int i = 0; i < gridindex.length; i++) {
			if (x<gridindex[i]){
				return i;
			}
	...
	static public int getYindex(float y){
		for (int i = 0; i < gridindex.length; i++) {
			if (y<gridindex[i]){
				return i;
			}
	...
	static private void runTrade() {
        ...
        for (MapIndex mapIndex : listeIndexPossible) {
            ...
            for (int i = 0; i < diverse; i++) {
                bestbesoinindice=0;
                bestoffreindice=0;
                concatenateSortedOffreList1 =new Array<>();
                concatenateSortedOffreList2 =new Array<>();
	...
	static private void makeTrade(Besoin besoin,Offre offre) {
        ...
        if (besoin.getPrixmax()<offre.getPrix()){
            bestbesoinindice++;
            return;
        }
